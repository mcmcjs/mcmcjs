...
| --- | --- | --- |
| `run [input]` | full workflow: fit, diagnose, record a run (omit the input on a terminal to pick a model; `--report` opens the result) | `--data`, `--draws`, `--warmup`, `--chains`, `--adapt-delta`, `--seed`, `--backend`, `--entry`, `-o/--out`, `--stream-out`, `--prior`, `--algorithm`, `--thin`, `--adtype`, `--evaluation-mode`, `--monitor`, `--timeout`, `--parallel`, `--refit`, `--daemon`, `--julia-version`, `--package`, `--store` |
