...
- 6bc169b: Fit `.bugs` programs directly, marginalize discrete finite latents for a gradient sampler when the spec sets no evaluation mode, and ignore starting values given for observed or deterministic nodes instead of refusing them.
...
- 3a92e43: Pass an unobserved data entry to both Julia PPLs as `missing`, promoting the array's observed entries to Float64 so a continuous latent is not handed an integer slot to live in
...
- ce4801f: The driver runs HMC, HMCDA, and MH with per-algorithm options, selects the AD backend from the spec (ForwardDiff, ReverseDiff, or Mooncake; ReverseDiff joins the managed package set), thins draws, and seeds chains from named initial values.
